    ...
    const fetchData = async () => {
      try {
        ...
        if (profileData.stripeAccountId && !profileData.stripeOnboardingComplete) {
            // Potentially fetch updated status or prompt user
            // For now, we'll re-fetch profile after attempting status check
                ...
                <div key={campaign.id} className="bg-white rounded-2xl border shadow-sm overflow-hidden hover:shadow-md transition-shadow">
                  <div className="p-6">
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <span className={`h-2 w-2 rounded-full ${campaign.status === 'ACTIVE' ? 'bg-green-500' : campaign.status === 'COMPLETED' ? 'bg-blue-500' : 'bg-gray-400'}`} />
                    </div>
