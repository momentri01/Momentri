...
export const register = async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(201).json({ token, user: { id: user.id, email: user.email, fullName: user.fullName, role: user.role, country: user.country, province: user.province, businessName: user.businessName } });
  } catch (error: any) {
